...
const moji = require('./../../../../data/emoji.json')
...
module.exports.run = async (client, player, track) => {
    ...
    const titles = track.info.title.length > 50 ? track.info.title.substr(0, 50) + "..." : track.info.title;
    const authors = track.info.author.length > 50 ? track.info.author.substr(0, 50) + "..." : track.info.author;
    ...
    const filter = (message) => {
        if (message.guild.members.me.voice.channel && message.guild.members.me.voice.channelId === message.member.voice.channelId)
            ...
            message.reply({
                content: `${moji.no} You must be on the same voice channel as me to use this button.`,
                ephemeral: true,
            });
            return false;
        }
    ...
    collector.on("collect", async (message) => {
        if (message.customId === "loop") {
            ...
            if (!player) {
                ...
                const embed = new EmbedBuilder().setColor(client.color).setDescription(`${emoji.no} Song can't be replay`);
            ...
            if (!player) {
                ...
                const embed = new EmbedBuilder().setDescription(`${moji.no} This is the end of the queue! There is no more songs to be played.`).setColor(client.color);
            ...
            if (!player) {
                ...
                const embed = new EmbedBuilder().setDescription(`${moji.no} Sorry but I forgot what the previous song was so I can't play it.`).setColor(client.color);
            ...
            if (!player) {
                ...
                const embed = new EmbedBuilder().setDescription(`${moji.no} Volume can't be lower than **10%**`).setColor(client.color);
            ...
            if (!player) {
                ...
                const embed = new EmbedBuilder().setDescription(`${moji.no} Volume can't be higher than: **100%**`).setColor(client.color);
            ...
            if (!player) {
                ...
                // console.log('------')
                // console.log(player.currentTrack.info)
