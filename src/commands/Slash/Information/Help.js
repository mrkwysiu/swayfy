...
const { supportUrl, inviteUrl, imageUrl, token } = require("../../../settings/config.js");
...
module.exports = {
    ...
    run: async (client, interaction) => {
        ...
        const embed = new EmbedBuilder()
            ...
            //.setImage(imageUrl)
            .setDescription(
                `Hello **${interaction.member}**, I'm ***Swayfy***. A Rich Quality Music Bot. Support  Spotify & SoundCloud. Find out what I can do and let's go listen to music!`,
            )
            .setFooter({
        ...
        interaction.editReply({ embeds: [embed], components: [row, row2] }).then(async (msg) => {
                ...
                if (m.isStringSelectMenu()) {
                    if (m.customId === "help-category") {
                        ...
                        const embed = new EmbedBuilder()
                            ...
                            .setDescription(
                                `These are all available commands for this category. Just click these commands below tu use them.\n\n**❯ ${
                                    directory.slice(0, 1).toUpperCase() + directory.slice(1)
                                }:**\n${client.slashCommands
                                    .filter((c) => c.category === directory)
                                    .map((c) =>{
                                        const matchingCommand = commands.find(cmd => cmd.name === c.name)?.id;
                                        return `</${c.name}:${matchingCommand}> : *${c.description}*`
                                    })
                                    .join("\n")}`,
                            )
                            .setColor(client.color)
            ...
            collector.on("end", async (collected, reason) => {
                if (reason === "time") {
                    ...
                    const helpcmd = commands2.find(cmd => cmd.name === 'help')?.id;
                    const timed = new EmbedBuilder()
                        ...
                        .setDescription(
                            `Help Command Menu was expired.\nUse the </help:${helpcmd}> command to open new help menu.`,
                        )
                        //.setImage(imageUrl)
                        .setColor(client.color)
                        .setFooter({
